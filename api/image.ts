      ...
      if (dalleError.code === 'content_policy_violation') {
        ...
        const sanitizationResponse = await openai.chat.completions.create({
          ...
          messages: [
            {
              role: "system",
              content: "You are a content safety assistant. Your job is to make prompts safe for children aged 8-14 while keeping them COOL, EXCITING, and EPIC - never childish or babyish. If description is very violent, slightly reduce its intensity. Strictly prohibit any nudity, sexual content, suggestive clothing, romantic posing, kissing, or adult themes; characters must be fully modest and age-appropriate. Keep it cinematic, dramatic, and thrilling. Return ONLY the sanitized prompt, nothing else."
            },
            {
              role: "user",
